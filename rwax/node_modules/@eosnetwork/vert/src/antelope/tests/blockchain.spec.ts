  ...
  describe('eosio.token', () => {
    ...
    it('create: negative_max_supply', async () => {
      try {
        ...
        expect(e.message).to.equal(antelopeAssert('max-supply must be positive'));
      }
    });
    ...
    it('create: symbol_already_exists', async () => {
      ...
      try {
        ...
        expect(e.message).to.equal(antelopeAssert('token with symbol already exists'));
      }
    });
    ...
    it('create: max_supply', async () => {
      ...
      try {
        ...
        expect(e.message).to.equal(antelopeAssert('invalid supply'));
      }
    });
    ...
    it('issue', async () => {
      ...
      try {
        ...
        expect(e.message).to.equal(antelopeAssert('quantity exceeds available supply'));
      }

      try {
        await eosioToken.actions.issue(['alice', '-1.000 TKN', 'hola']).send('alice@active');
      } catch (e) {
        expect(e.message).to.equal(antelopeAssert('must issue positive quantity'));
      }
    ...
    it('transfer', async () => {
      ...
      try {
        ...
        expect(e.message).to.equal(antelopeAssert('overdrawn balance'));
      }

      try {
        await eosioToken.actions.transfer(['alice', 'bob', '-1000 CERO', 'hola']).send('alice@active');
      } catch (e) {
        expect(e.message).to.equal(antelopeAssert('must transfer positive quantity'));
      }
    });
  });
});
