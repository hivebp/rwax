      ...
      env: {
        ...
        */
        ...
        eosio_assert: (test: i32, msg: ptr): void => {
          ...
          if (!test) {
            throw new Error(antelopeAssert(this.memory.readString(msg)))
          }
        },
        eosio_assert_message: (test: i32, msg: ptr, msg_len: i32): void => {
          // log.debug('eosio_assert_message');
          if (!test) {
            throw new Error(antelopeAssertMessage(this.memory.readString(msg, msg_len)))
          }
        },
        eosio_assert_code: (test: i32, code: i64): void => {
          // log.debug('eosio_assert_code');
          if (!test) {
            throw new Error(antelopeAssertMessage(antelopeAssertCode(BigInt.asUintN(64, code))))
          }
        },
